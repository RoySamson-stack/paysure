...
router.get('/download', auth, async (req, res) => {
  try {
    ...
    // Get statement data (reuse logic from above)
    const months = parseInt(period) || 3;
    const startDate = new Date();
    startDate.setMonth(startDate.getMonth() - months);
    ...
    doc.text(`Account Holder: ${userData.phone_number}`, 50, 100);
    ...
    doc.fontSize(12)
       ...
       .text(`Total Amount Deposited: KES ${parseFloat(summary.total_amount || 0).toLocaleString()}`, 50, 210)
       .text(`Average Deposit: KES ${parseFloat(summary.average_amount || 0).toLocaleString()}`, 50, 230);
    
    // Transactions
    doc.fontSize(16).text('TRANSACTION HISTORY', 50, 270);
